...
<p >
	...
	<tr><td class="leftSide"><strong><? echo($texts['auction_ferkel_stalls']) ;?>:</strong></td><td id="stalls" class="rightSide"><? echo($gBase->CurrentAuction["metadata"]["auction_stalls"]);?></td></tr>
		...
		<tr><td class="leftSide"><strong><? echo($texts['auction_ferkel_health']) ;?>:</strong></td><td id="health" class="rightSide"><? echo($gBase->CurrentAuction["metadata"]["auction_health"]);?></td></tr>
<tr><td class="leftSide"><strong><? echo($texts['auction_ferkel_status']) ;?>:</strong></td><td id="status" class="rightSide"><? echo($gBase->CurrentAuction["metadata"]["auction_status"]);?></td></tr>
...
function getRunningAuction(){
  ...
  $.getJSON("index.php", { "action": "get_running_auction", "view": "show_running_auction", "mode":"ajax", "category_id":"2", "county_id":"<? echo($_REQUEST["county_id"]); ?>","auction_id":currentAuctionID, "sid":"<? echo($_COOKIE["PHPSESSID"]); ?>"},
...
function submitBid(){
	...
	if($gBase->User!=null){
		...
		if($gBase->User["is_buyer"]=="no"){
  		...
  		$.getJSON("index.php", { "action": "bid_on_running_auction", "view": "show_running_auction", "mode":"ajax", "category_id":"2","county_id":"<? echo($_REQUEST["county_id"]); ?>","auction_id":currentAuctionID,"bid":$("#your_bid").val(), "sid":"<? echo($_COOKIE["PHPSESSID"]); ?>"},
...
function displayResponse(data){
								...
								//if($("#curent_price").html()!=data.current_auction.current_entity_price){
									...
									$("#your_bid").empty();
										for(i=1;i<11;i++){
											nextBid=Runden3Dezimal(currentbid+0.05*i);
								...
								$("#end_time").html(data.current_auction.supposed_end_time.substr(10,10));
								...
								$("#genes").html(data.current_auction.metadata.auction_genes);
								$("#origin").html(data.current_auction.metadata.auction_origin);
								$("#stalls").html(data.current_auction.metadata.auction_stalls);
								$("#health").html(data.current_auction.metadata.auction_health);
								$("#status").html(data.current_auction.metadata.auction_status);
