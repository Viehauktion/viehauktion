...
function getRunningAuction(){
  ...
  $.getJSON("index.php", { "action": "get_running_auction", "view": "show_running_auction", "mode":"ajax", "category_id":"1", "county_id":"<? echo($_REQUEST["county_id"]); ?>","auction_id":currentAuctionID, "sid":"<? echo($_COOKIE["PHPSESSID"]); ?>"},
...
function submitBid(){
	...
	if($gBase->User!=null){
		...
		if($gBase->User["is_buyer"]=="no"){
  		...
  		$.getJSON("index.php", { "action": "bid_on_running_auction", "view": "show_running_auction", "mode":"ajax", "category_id":"1","county_id":"<? echo($_REQUEST["county_id"]); ?>","auction_id":currentAuctionID,"bid":$("#your_bid").val(), "sid":"<? echo($_COOKIE["PHPSESSID"]); ?>"},
